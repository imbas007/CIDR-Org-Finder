...
async def process_ip(session, ip, org_name, semaphore, verbose=False):
    try:
        ...
        for cidr in all_cidrs:
            org = None
            asn = cymru_data.get('asn') if cymru_data else "N/A"
            ...
            if org:
                results.append((cidr, org, asn))  # Include ASN here
                if verbose:
                    print(f"Added result: {cidr} ({org}, ASN: {asn})", file=sys.stderr)
...
async def find_subnets(ip_list, org_name=None, verbose=False):
    ...
    if org_name:
        return [(cidr, org, asn) for sublist in results if isinstance(sublist, list) for cidr, org, asn in sublist if org_name.lower() in org.lower()]
    else:
...
async def main():
    ...
    for cidr, org, asn in results:  # Adjusted to handle ASN
        org_grouped[org].add((cidr, asn))
    
    for org in sorted(org_grouped.keys()):
        cidrs = sorted(org_grouped[org], key=lambda x: ipaddress.ip_network(x[0]).num_addresses)
        for cidr, asn in cidrs:
            print(f"{cidr} (ASN: {asn}, {org})")  # Print ASN along with CIDR and organization
